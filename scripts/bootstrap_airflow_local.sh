...
echo "2/6 Initialize Airflow metadata DB"
./scripts/init_airflow_metadata_db.sh

echo "3/6 Pull Airflow image"
docker pull "${AIRFLOW_IMAGE}"

echo "4/6 Prepare Airflow state volume"
./scripts/prepare_airflow_state.sh

echo "5/6 Run Airflow migrations"
./scripts/airflow_compose.sh up --no-deps --abort-on-container-exit airflow-init

echo "6/6 Start Airflow services"
